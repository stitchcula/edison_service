...
export default class Mosca {
    ...
    async onReady(){
        this.server.authenticate =async function(client, uin, token, callback) {//online login
            var res=await this.mongo.collection("users").findOne({token,uin})
            if(res){
                res=await this.mongo.collection("online_list")
                    .insertOne({session:client.id,uin,time:new Date().getTime()})
